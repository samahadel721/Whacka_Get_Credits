...
```bash
df -h ~                              # ملف full محتاج ~650MB فاضية
...
**حارس المساحة:** `setup.sh` بيقرا `df` قبل أي تنزيل، ولو الفاضي أقل من الحد (`tools` 117MB · `python` 215MB · `minimal` 293MB · `full` 635MB) بيوقف ويشرح الخيارات — لأن انقطاع التنزيل في النص بيسيب `dpkg` مكسورًا. للتخطّي: `TD_FORCE=1 bash scripts/setup.sh` أو `bash install.sh --force`. في وضع `-y` بلا `TD_FORCE` الأمر بيرفض (رقم خروج 1) عشان الـ CI ما يكمّلش على جهاز ناقص.
