...
**السطر ده بالظبّه = ملف `full`** (17 حزمة: Node + Python + أدوات). قبلها شوف الفاضي عندك:

```bash
df -h ~                      # ملف full محتاج ~650MB فاضية (تنزيل + فك + كاش)
bash install.sh              # full — الافتراضي
bash install.sh --full       # نفس الشيء صراحةً
```

`setup.sh` بيقيس المساحة بنفسه و**بيرفض** يبلّش التنزيل لو الفاضي أقل من المطلوب (عشان النصف-مثبّت بيكسر `dpkg`). لو واثق إن عندك مساحة: `bash install.sh --force`.
...
| `scripts/counter.sh <URL> [--tries N] [--bust] [--json]` | يفحص **عداد الزوار في موقعك**: يوصل للصفحة؟ فيه عنصر عدّاد في الـ HTML؟ الرقم بيتحرك مع كل طلب ولا لأ؟ بيكشف كمان خدمات الإحصاء (GA4/Plausible/Histats…) وهيدرات الكاش اللي بتجمّد الرقم. **قراءة فقط** — لا يولّد زيارات. |
...
## عدّاد الزوار في موقعك — بيتحرك ولا لأ؟

لو حطيت في موقعك عدّاد زيارات وعايز تتأكد إنه شغال فعليًا (مش مجرد رقم ثابت)، السطر ده بيغطي الفحص كله:

```bash
bash scripts/counter.sh https://example.com          # 3 طلبات ويطلعلك الحكم
bash scripts/counter.sh http://192.168.1.20:8080 --tries 6 --gap 2
bash scripts/counter.sh https://example.com --bust    # يكسر كاش الـ CDN (لو الرقم كان متجمّد)
```

| رقم الخروج | معناه | تعمل إيه |
|---|---|---|
| 0 | لقيت عدّاد وبيتحرك مع كل طلب | تمام ✓ |
| 3 | العدّاد موجود بس ثابت | فيه كاش (`--bust`)، أو بيعدّ زائر/IP مرة في اليوم، أو بيعتمد على كوكيز المتصفح — افتحها في نافذة تصفح خفي وقارن |
| 1 | الصفحة وصلت ومفيش عدّاد في الـ HTML | الكود مش بيتطبع في الرد (قالب تاني؟)، أو الرقم بيُحقن بالـ JS. لو فيه GA4/Plausible فالرقم في لوحة التحكم مش في الصفحة |
| 5 | عنصر العدّاد موجود بس **فاضي** في الرد | سكربت التعبئة فشل أو الـ API بيرجع خطأ — `curl -i https://موقعك/api/counts` وافتح الكونسول في المتصفح |
| 2 | مفيش وصول | راجع الرابط/المنفذ وشغّل السيرفر: `bash scripts/serve.sh ~/projects/demo` |

> ملاحظة مهمة: الأداة بتقرأ الردّ زي ما المتصفح بيعمله بالضبط (3–6 طلبات). مش بتولّد زيارات ولا بتزوّد أرقام — لو عايز تعدّي زوّارك بيدك ده يبقى تزوير إحصاء، ومفيش أداة هنا بتعمله. عايز تشوف الأرقام الحقيقية؟ `G-XXXX` اللي بيطلعه الفحص بيوصلك لتقرير GA4/Search Console حقيقي.
